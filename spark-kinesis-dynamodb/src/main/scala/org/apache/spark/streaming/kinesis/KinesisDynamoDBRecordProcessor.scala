...
/** Kinesis-specific implementation of the Kinesis Client Library (KCL) IRecordProcessor. This
  * implementation operates on the Array[Byte] from the KinesisReceiver. The Kinesis Worker creates
  * an instance of this KinesisRecordProcessor for each shard in the Kinesis stream upon startup.
  * This is normally done in separate threads, but the KCLs within the KinesisReceivers will balance
  * themselves out if you create multiple Receivers.
  *
  * @param receiver
  *   Kinesis receiver
  * @param workerId
...
private[kinesis] class KinesisDynamoDBRecordProcessor[T](
  ...
  /** The Kinesis Client Library calls this method during IRecordProcessor initialization.
    *
    * @param shardId
    *   assigned by the KCL to this particular RecordProcessor.
    */
  ...
  /** This method is called by the KCL when a batch of records is pulled from the Kinesis stream.
    * This is the record-processing bridge between the KCL's IRecordProcessor.processRecords() and
    * Spark Streaming's Receiver.store().
    *
    * @param batch
    *   list of records from the Kinesis stream shard
    * @param checkpointer
    *   used to update Kinesis when this batch has been processed/stored in the DStream
    */
    ...
    if (!receiver.isStopped()) {
      try {
        ...
        for (start <- 0 until batch.size by maxRecords) {
          ...
          logDebug(
            s"Stored: Worker $workerId stored ${miniBatch.size} records " +
  ...
  /** Kinesis Client Library is shutting down this Worker for 1 of 2 reasons: 1) the stream is
    * resharding by splitting or merging adjacent shards (ShutdownReason.TERMINATE) 2) the failed or
    * latent Worker has stopped sending heartbeats for whatever reason (ShutdownReason.ZOMBIE)
    *
    * @param checkpointer
    *   used to perform a Kinesis checkpoint for ShutdownReason.TERMINATE
    * @param reason
...
private[kinesis] object KinesisDynamoDBRecordProcessor extends Logging {
  ...
  /** Retry the given amount of times with a random backoff time (millis) less than the given
    ...
    * @throws Unretryable
    *   exception, unexpected exception, or any exception that persists after numRetriesLeft reaches
    *   0
    */
  @annotation.tailrec
